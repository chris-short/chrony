...
echo "  docker run -d --name chrony --cap-drop=ALL --cap-add=SYS_TIME --read-only \\"
echo "    -v chrony-data:/var/lib/chrony -v chrony-run:/run/chrony -v chrony-log:/var/log/chrony \\"
echo "    -p 123:123/udp ghcr.io/chris-short/chrony:latest"
echo ""
